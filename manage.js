    ...
    var html = todoList.map(function (item, index) {
      return `
              <tr>
                          ...
                          <td>
                  <input  type="button" value="EDIT" class="edit1" onClick=edit(${
                    item.id
                  })>
                  <input class="delete" type="button" value="DELETE" onClick=deletes(${
                    item.id
                  })>
                  <button id="del${
                    item.id
                  }" class="Finish" onClick=finish(${item.id})>FINISH</button></td>
                      </tr>
              `;
    });
    ...
    return `
                  <tr>
                          ...
                          <td>
                            ...
                            <input type="button" value="Delete" class="delete"  onClick=deletes(${
                              item.id
                            })>
                            <button id="del${
                              item.id
                            }" class="Finish" onClick=finish(${item.id})>Finish</button>
                        </td>
                  </tr>
          `;
  });
...
function edit(idtodolist) {
  ...
  var html = `<input id="edittext" type="text" value="" placeholder="Enter a new task here"> <input id="submit" type="button" value="EDIT" onclick="submit()">`;
